    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center flex-wrap gap-4">
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight">Daftar Tahun Ajaran</h1>
                    <p class="text-blue-100 text-sm mt-1">Kelola data periode tahun ajaran aktif.</p>
                </div>
                <a href="{{ route('admin.tahun_ajaran.create') }}" class="inline-flex items-center px-5 py-2.5 bg-blue-600 border border-transparent rounded-xl shadow-md text-sm font-bold text-white hover:bg-blue-700 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Tahun Ajaran</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse($data as $index => $ta)
                                <tr class="hover:bg-blue-50/50 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $index + 1 }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">{{ $ta->tahun_ajaran }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('admin.tahun_ajaran.edit', $ta->id) }}" class="inline-flex items-center px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors">
                                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                Edit
                                            </a>
                                            <form action="{{ route('admin.tahun_ajaran.destroy', $ta->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus tahun ajaran ini?');">
                                                @csrf
                                                @method('DELETE')
